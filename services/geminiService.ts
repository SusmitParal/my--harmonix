...
// 2. Fix the TypeScript/Vite Env Error
// We use a type cast so TypeScript doesn't crash during build
...
export const generateLyrics = async (song: string, artist: string): Promise<string> => {
  ...
  try {
    // Correct Model Name for stability
...
export const generateVibeQuery = async (currentSong: Song, languages: string[]): Promise<string> => {
    ...
    try {
        ...
        const prompt = `Next song mood after "${currentSong.title}" by "${currentSong.artist}". Language: ${langStr}. Return ONLY a 3-word search query.`;
...
export const getArtistBio = async (artistName: string): Promise<ArtistInfo> => {
  ...
  try {
    ...
    const prompt = `Provide a short bio for "${artistName}" and 3 top hits. Format as JSON: {"bio": "...", "topHits": [{"title": "...", "album": "..."}]}`;
